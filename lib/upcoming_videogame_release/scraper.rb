...
class UpcomingVideogameRelease::Scraper
  ...
  def self.create_game_hash(game)
    console = UpcomingVideogameRelease::Console.find_or_create_by_name(game.css("span.item-platform").text)
    genres = []
    game.css("span.item-genre").text.split(",").each do |genre|
      genres << UpcomingVideogameRelease::Genre.find_or_create_by_name(genre.strip)
    end
    hash = {
  ...
  def self.create_games_with_hash
    get_games.each do |game|
      UpcomingVideogameRelease::Game.create_or_update_by_hash(create_game_hash(game))
    end
  end
